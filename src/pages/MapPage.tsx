...
const MapPage = () => {
  ...
  const { data: products = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("products")
        .select("id, title, description, image_url, latitude, longitude, category")
  ...
  const { data: requests = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("id, title, description, image_url, latitude, longitude, type")
        .not("latitude", "is", null)
        .not("longitude", "is", null);
      return data || [];
    },
  ...
  const { data: profiles = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("profiles")
        .select("user_id, username, display_name, avatar_url, latitude, longitude")
        .not("latitude", "is", null)
        .not("longitude", "is", null);
      return data || [];
    },
      ...
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          <h1 className="font-display text-3xl font-bold mb-2">
            <MapPin className="w-7 h-7 inline mr-2 text-primary" />
            {t("map.title")}
          </h1>
        ...
        <div className="flex gap-6 mt-4 justify-center text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full bg-blue-500" /> {t("nav.products")}
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full bg-emerald-500" /> {t("exchange.service")}
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full bg-purple-500" /> {t("nav.users")}
          </span>
        </div>
      </div>
    </div>
